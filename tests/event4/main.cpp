...
struct progress : boost::noncopyable {
        ...
        void engine(){ e.notify(time); }

        progress(const sc_time& time)
                : time(time)
        {
...
struct actor : boost::noncopyable {
        ...
        void operator()(){
                sc_spawn_options opt;
                opt.set_sensitivity(trigger);
                if(!is_thread) opt.spawn_method();
                sc_spawn(boost::bind(&actor::go,a,report,is_thread),name.c_str(),&opt);
        }

        static void go(action a, action report, bool is_thread){
                if(is_thread)
                        while(1){
                                a();
                                report();
...
int sc_main(int argc, char** argv){
       if(argc!=9){ print("Wrong argc, expected 8 parameters, usage example: " << argv[0] << " 1 5 4 4 0 0 0 timed:1"); exit(1); }
        ...
        size_t sim_time          = atoi(argv[2]);               // simulation time
        ...
        progress p(sc_time(period,SC_MS));
